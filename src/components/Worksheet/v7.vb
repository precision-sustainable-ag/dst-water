...
C.Connection
 ...
 ' get max depth of soil
   strSQL = "select [soilFile], max([bottom depth]) as depth from [Soil$] where [soilFile] = '" & _
     strSoilFile & "' group by [soilFile]"
C.rs2.Open strSQL, C.cn, adOpenStatic
depth = C.rs2("depth") - C.rs("yseed")
C.rs2.Close

 
RowSP = C.rs("RowSpacing")
density = C.rs("population") / 10000
...
Print #1, C.rs("lat"), C.rs("long"), C.rs("altitude")
...
strSQL = "Select RowSpacing from [Init$] where ID ='" & idStr & "'"
C.rs2.Open strSQL, C.cn, adOpenStatic

Print #1, "Row Spacing"
Print #1, C.rs2("RowSpacing")
...
strSQL = "select rowspacing, sowing from [Init$] where ID ='" & idStr & "'"
C.rs.Open strSQL, C.cn
MaxX = C.rs("RowSpacing") / 2
SowDate = C.rs("sowing")
C.rs.Close
...
strSQL = "select ID, [amount] , depth, L_C, L_N, M_C, M_N, date from [Fertilization$] where ID='" & idStr & "'"
C.rs.Open strSQL, C.cn, adOpenStatic
' rs now contains fertilization info
'now get count of fert times
 ...
 ' now  start writing fertilization info into the file
   ...
   For i = 1 To fertCount
    'fert data are in the rs record set
     ...
     depth = C.rs("depth")
     L_C = C.rs("L_C") * factor / 10000 * 1000000#   'litter
     L_N = C.rs("L_N") * factor / 10000 * 1000000#
     M_C = C.rs("M_C") * factor / 10000 * 1000000#   'manure
     M_N = C.rs("M_N") * factor / 10000 * 1000000#
     Date1 = Format(C.rs("Date"), "mm/dd/yyyy")
...
Print #1, BasePath + "\Water.DAT"
Print #1, BasePath + "\WaterBound.DAT"
...
Sub WriteWater_bnd(idStr As String)
' get id into string
Dim WaterFile, WaterBnd As Variant
Set C = New Class1
C.Connection
...
FilePath = C.FilePathRoot + "\" + C.rs("Path") + "\water.dat"
  Open FilePath For Output As #1
  
 Print #1, "*** WATER MOVER PARAMETERINFORMATION ***************************"
 Print #1, "MaxIt   TolTh TolH    hCritA       hCritS      DtMx  htab1   htabN EPSI.Heat  EPSI.Solute"
 Print #1, "   20     0.01  0.05  -1.00000E+005 1.0E+010       0.02 0.001   1000     0.5        0.5"
 Print #1,
 
 Close #1
 ' now do water boundary file
 FilePath = C.FilePathRoot + "\" + C.rs("Path") + "\WaterBound.dat"
  Open FilePath For Output As #1

Print #1, "*** WATER MOVER TIME-DEPENDENT BOUNDARY"
Print #1, " Time  Node  VarB"
Print #1, " 252.542"
Print #1, "  6 0.000000E+000"
Print #1, "   7 0.000000E+000"
Print #1, "   12 0.000000E+000"
Print #1, "   13 0.000000E+000"
Print #1,
